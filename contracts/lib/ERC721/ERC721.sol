...
contract ERC721 is IERC721, IERC721Metadata {
    ...
    function getApproved(uint256 tokenId) external view override returns (address operator) {
        require(_tokenApprovals[tokenId] != address(0), "token do not exist");
    ...
    function approve(address to, uint256 tokenId) external override {
        ...
        address owner = _owners[tokenId]; // 保存地址，避免多次读取链上数据，减少 gas 消耗
     ...
     * 条件:
     * 1. tokenId尚不存在：没有人拥有过这个 tokenId。
     * 2. to不是0地址.
     */
    function _mint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "mint to zero address"); // to不是0地址
        require(_owners[tokenId] == address(0), "token already minted"); // 没有人拥有过这个 tokenId
        _balances[to] += 1;
        _owners[tokenId] = to;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0
            ? string(abi.encodePacked(baseURI, tokenId.toString())) // 不与合约交互用encodePacked
            : "";
    }
